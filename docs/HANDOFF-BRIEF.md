...
3. **Google Drive real (opcional, no bloqueante):** la sección `/workspace` es un launcher de links (el "APIs No conectado" es POR DISEÑO, no bug). La integración real (`src/lib/google-drive.ts`) usa Service Account read-only. Martín debe crear el Service Account en Google Cloud + compartir carpeta + bajar JSON key; luego cargar 3 env vars en Netlify: `GOOGLE_CLIENT_EMAIL`, `GOOGLE_PRIVATE_KEY`, `GOOGLE_DRIVE_FOLDER_ID`.
