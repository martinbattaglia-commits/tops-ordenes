...
describe("T-C4-01 · ROLLBACK: la vía coordinada está cerrada", () => {
  ...
  it("control sano: 200 entradas, 194 ejecutables y 6 no ejecutables, recalculados", () => {
    expect(REAL.entries).toHaveLength(200);
    const ejec = REAL.entries.filter(
      (e) => e.estado === "active" || e.estado === "corrective",
    );
    expect(ejec).toHaveLength(194);
    expect(REAL.entries.length - ejec.length).toBe(6);
    // Y los contadores declarados coinciden con lo recalculado.
    expect(REAL.ejecutables).toBe(194);
    expect(REAL.no_ejecutables).toBe(6);
  });
  ...
  it("MUTANTE B · reclasificación CON contadores maquillados (195/5) → sigue FAIL por rollback", () => {
    const c = clon();
    const i = c.entries.findIndex((e) => e.filename === OBJETIVO);
    c.entries[i].estado = "active";
    c.ejecutables = 195;
