...
const CARTERA: ReadonlyArray<{
  ...
  { razon: "DISTRIBUIDORA  ÁVILA S.A.", cuit: "30-71234567-8", nombre_comercial: "Ávila Logística", codigo: "AVL", email: "compras@avila.test", telefono: "11 5555-0001", activo: true },
  { razon: "Farmacéutica del Plata SRL", cuit: "30712345679", nombre_comercial: "FarmaPlata", codigo: "FDP", email: "info@farmaplata.test", telefono: "11 5555-0002", activo: true },
  ...
  { razon: "Transportes Avellaneda SA", cuit: "30-71234567-1", nombre_comercial: null, codigo: null, email: null, telefono: "30712345679", activo: true },
  { razon: "Comercial Retirada SRL", cuit: "30-71234567-2", nombre_comercial: null, codigo: "RET", email: null, telefono: null, activo: false },
];
...
describe("T-CLI-A1-01 · acotamiento del límite", () => {
  beforeAll(async () => {
    ...
    await db.query(`
      ...
      select 'Masiva ' || g, '20' || lpad(g::text, 9, '0') || '3'
...
describe("T-CLI-A1-01 · advertencia de duplicado", () => {
  it("detecta CUIT idéntico aunque el formato difiera", async () => {
    const { rows } = await db.query<{ match_reason: string; razon: string }>(
      "select match_reason, razon from public.clients_duplicate_candidates($1,$2)",
      ["Nombre Totalmente Distinto SA", "30712345678"],
    );
  ...
  it("advierte por razón social parecida, sin CUIT", async () => {
    const { rows } = await db.query<{ razon: string; match_reason: string }>(
      "select razon, match_reason from public.clients_duplicate_candidates($1,$2)",
      ["Distribuidora Avila S.A.", null],
    );
...
describe("T-CLI-A1-01 · invariantes del registro maestro", () => {
  it("la identidad externa es única por sistema", async () => {
    await db.query(
      "update public.clients set external_source='clientify', external_id='777' where codigo='AVL'",
    );
    let fallo: string | null = null;
    try {
      await db.query(
        "update public.clients set external_source='clientify', external_id='777' where codigo='FDP'",
      );
    } catch (e) {
      fallo = e instanceof Error ? e.message : String(e);
    }
    expect(fallo).toMatch(/clients_external_uk/);
  });

  it("el índice de identidad externa es PARCIAL: admite muchas filas sin identidad", async () => {
    // Se INSERTAN dos filas nuevas con external_source/external_id nulos. Si
    // el índice fuese total en vez de parcial, la segunda fallaría.
    await db.query(
      "insert into public.clients (razon, cuit) values ('Sin Externa Uno SA','30-88888888-1')",
    );
    await expect(
      db.query("insert into public.clients (razon, cuit) values ('Sin Externa Dos SA','30-88888888-2')"),
  ...
  it("el código es único sin distinguir mayúsculas", async () => {
    ...
    try {
      await db.query(
        "insert into public.clients (razon, cuit, codigo) values ('Otra SA','30-99999999-9','avl')",
      );
    } catch (e) {
      fallo = e instanceof Error ? e.message : String(e);
    }
  ...
  it("sync_state sólo admite los estados declarados", async () => {
    let fallo: string | null = null;
    try {
      await db.query("update public.clients set sync_state='inventado' where codigo='AVL'");
    } catch (e) {
      fallo = e instanceof Error ? e.message : String(e);
    }
    expect(fallo).toMatch(/clients_sync_state_chk/);
  });
...
describe("T-CLI-A1-01 · frontera de seguridad de client_events", () => {
  beforeAll(async () => {
    ...
    await db.query(
      `insert into public.profiles (id, role, client_id) values
         ($1,'operaciones',null), ($2,'cliente',$4), ($3,'cliente',$5)`,
      [INTERNO, CLIENTE_A, CLIENTE_B, avl, fdp],
    );
    // Dos eventos, uno por cliente. Se insertan como dueño (sin RLS forzada
    // para el insert de preparación no aplica: FORCE sí aplica, así que se
    // hace con el observador interno).
  ...
  it("un usuario de portal sólo ve los eventos de SU cliente", async () => {
  ...
  it("un interno ve la auditoría completa", async () => {
  ...
  it("la tabla lleva FORCE row level security", async () => {
  ...
  it("la atribución NO se puede forjar: el trigger la sella", async () => {
    ...
    const forjado = await comoUsuario(INTERNO, async () => {
      await db.query(
    ...
    expect(forjado.actor).toBe(INTERNO);
