...
export const ORIGEN = {
  ...
  clientsWrite: "0001_init.sql:205-208 (policy clients write internal)",
  rbacTablas: "0009_rbac.sql (roles, role_permissions, user_roles)",
  hasPermission: "0009_rbac.sql:164-175 (incluye la rama current_role()='admin')",
} as const;
...
export const CIERRE_ACOTADO = `
  ...
  -- 0001_init.sql:205-208. Es la policy de la que depende MATERIALMENTE que
  -- una RPC pueda escribir clients sin BYPASSRLS. Omitirla dejaba el
  -- constructo apoyado en que el harness corre como superusuario.
  ...
  -- condicion_iva_t — 0011_arca_billing.sql:14. La usa client_create.
  create type public.condicion_iva_t as enum (
    'RESPONSABLE_INSCRIPTO','MONOTRIBUTO','EXENTO',
    'CONSUMIDOR_FINAL','NO_RESPONSABLE','NO_CATEGORIZADO'
  );
  alter table public.clients
    add column condicion_iva public.condicion_iva_t not null default 'RESPONSABLE_INSCRIPTO',
  ...
  -- RBAC minimo — 0009_rbac.sql. has_permission es la autorizacion que las
  -- RPC de 0242 consultan; sin ella no se puede medir la denegacion real.
  ...
  -- Copia FIEL de 0009_rbac.sql:164-175, INCLUIDA la rama de admin. Omitirla
  -- medía una funcion que no es la de produccion: en el sistema real un
  -- perfil admin obtiene toda capacidad por esa via, y una prueba de
  -- denegacion que no lo contemple da una seguridad que no existe.
