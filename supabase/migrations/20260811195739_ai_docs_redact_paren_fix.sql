-- =========================================================================
-- CORRECTIVA FORWARD-ONLY · `0176_knowledge_docs_projection.sql`
--
-- ─── QUÉ CORRIGE ────────────────────────────────────────────────────────
--
-- `0176` no aplica sobre una base nueva: `public.ai_docs_redact(text)` abre
-- SEIS `regexp_replace(` cuando sus propios comentarios enumeran CINCO capas
-- de redacción. El opener más externo no tiene capa asignada y queda sin
-- cerrar, así que el `end` del CASE llega con un paréntesis pendiente y
-- PostgreSQL corta con «syntax error at or near "end"».
--
-- Con `ON_ERROR_STOP` la migración entera revierte, y con ella TODO el
-- contrato de proyección documental: `ai_docs_redact`,
-- `ai_docs_visibility_key`, la vista `ai_docs_projection`, el dry-run, la
-- reproyección, el backfill y los seis triggers. De ahí cascadean `0177`,
-- `0178`, `0181`, `0183` y `0185`.
--
-- ─── POR QUÉ ES CORRECTIVA Y NO EDICIÓN DEL HISTÓRICO ───────────────────
--
-- `0176` YA ESTÁ APLICADA en producción y registrada en el ledger, y allí la
-- expresión está balanceada: editar el archivo histórico haría divergir el
-- repositorio de lo que realmente se aplicó. Se deja intacto y el contrato se
-- repone hacia adelante.
--
-- ─── QUÉ CONTIENE ───────────────────────────────────────────────────────
--
-- El cuerpo de `0176` VERBATIM, con UNA sola línea removida: el
-- `regexp_replace(` sobrante. Las cinco capas de redacción quedan intactas y
-- en el mismo orden. Todo es `create or replace` / `if not exists`, así que es
-- idempotente y no altera una base donde `0176` ya corrió.
-- =========================================================================
