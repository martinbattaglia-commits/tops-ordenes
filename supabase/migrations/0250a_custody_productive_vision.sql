...
begin
  ...
  -- La vista del QR NO afirma nada sobre el POD de una unidad física.
  --
  -- El literal `false` que había antes era una afirmación positiva equivocada:
  -- una unidad efectivamente entregada seguía informando que no tenía POD. La
  -- derivación que lo reemplazó era peor: una unidad física puede repartirse
  -- entre varias allocations, así que un solo despacho entregado marcaba POD
  -- para toda la unidad mientras el resto seguía en depósito. Ninguna de las
  -- dos podía sostenerse en la pantalla que la Adenda §4.2 designa como
  -- artefacto de identidad del bien.
  --
  -- Se retira la clave hasta que exista una definición correcta —con noción de
  -- cantidad, o acotada a la allocation—. Un dato ausente es admisible; una
  -- afirmación falsa sobre la unidad de custodia, no.
  return jsonb_build_object('scope','physical_unit','public_id',u.public_id,
    'status',c.state,'events',v_summary);
end;
$$;
