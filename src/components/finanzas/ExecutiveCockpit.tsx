...
export function ExecutiveCockpit({
  ...
  const maxMonthlyVal = Math.max(...visibleMonthly.map((m) => Math.max(m.income, m.expense)), 1);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 bg-bg-surface rounded-xl border border-stroke-soft shadow-2xs transition-colors">
          ...
          <p className="text-xs text-fg-muted mt-2">
            Facturación, fletes y servicios
          </p>
        </div>
        ...
        <div className="bg-bg-surface rounded-xl border border-stroke-soft shadow-2xs p-5 transition-colors flex flex-col justify-between">
          <div className="flex items-center justify-between pb-3 border-b border-stroke-soft mb-4">
            <div>
              ...
              <p className="text-xs text-fg-muted">Distribución porcentual y montos por rubro</p>
            </div>
            ...
            <div className="relative flex items-center justify-center p-2">
              <svg viewBox="0 0 160 160" className="w-36 h-36 transform -rotate-90">
                {donutSlices.length === 0 ? (
                  <circle cx="80" cy="80" r="60" fill="none" stroke="currentColor" strokeWidth="24" className="text-stroke-soft" />
                ) : (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-[10px] uppercase font-bold text-fg-muted">Total Gastos</span>
                <span className="text-xs font-bold text-fg-primary">{formatMoney(totalCategoryAmount || pnl.gastosOperativos.total)}</span>
              </div>
            </div>
        ...
        <div className="bg-bg-surface rounded-xl border border-stroke-soft shadow-2xs p-5 transition-colors">
          <div className="flex items-center justify-between pb-3 border-b border-stroke-soft mb-4">
            <div>
              ...
              <p className="text-xs text-fg-muted">Proveedores, retiros y acreedores con mayor desembolso</p>
            </div>
          ...
          <div className="space-y-3">
            {metrics.topPayees.length === 0 ? (
              <div className="py-8 text-center text-fg-muted text-xs">
                Sin movimientos de pago registrados para el período.
              </div>
            ) : (
      ...
      <div className="bg-bg-surface rounded-xl border border-stroke-soft shadow-2xs p-5 transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-stroke-soft mb-4">
          <div>
            ...
            <p className="text-xs text-fg-muted">Evolución mensual de facturación, costos y resultado neto operativo</p>
          </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 lg:grid-cols-12 gap-2 pt-2 items-end min-h-[180px]">
          {visibleMonthly.map((m) => {
            const incHeight = Math.max(10, Math.round((m.income / maxMonthlyVal) * 120));
            const expHeight = Math.max(10, Math.round((m.expense / maxMonthlyVal) * 120));
            ...
            return (
              <div key={m.period} className="flex flex-col items-center gap-1.5 group">
                ...
                <span className={`text-[9px] font-bold tabular-nums px-1 rounded ${
                  m.net >= 0 ? "text-status-success bg-emerald-50 dark:bg-emerald-950/60" : "text-tops-red bg-rose-50 dark:bg-rose-950/60"
                }`}>
        ...
        <div className="bg-bg-surface rounded-xl border border-stroke-soft shadow-2xs p-5 transition-colors space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-stroke-soft">
            <div>
              ...
              <p className="text-xs text-fg-muted">Cumplimiento presupuestario y desvíos</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3.5 bg-bg-surface-alt/60 rounded-xl border border-stroke-soft">
              ...
              <div className="text-xs text-fg-muted mt-1">Presupuestado: {formatMoney(metrics.budgetVsReal.budgetedIncome)}</div>
            ...
            <div className="p-3.5 bg-bg-surface-alt/60 rounded-xl border border-stroke-soft">
              ...
              <div className="text-xs text-fg-muted mt-1">Presupuestado: {formatMoney(metrics.budgetVsReal.budgetedExpense)}</div>
            ...
            {[
              ...
              <div key={v.label} className="p-3 bg-bg-surface-alt/60 rounded-xl border border-stroke-soft text-center">
                ...
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                  v.data.net >= 0 ? "bg-emerald-50 text-status-success dark:bg-emerald-950/60" : "bg-rose-50 text-tops-red dark:bg-rose-950/60"
                }`}>
        ...
        <div className="bg-bg-surface rounded-xl border border-stroke-soft shadow-2xs overflow-hidden transition-colors">
          <div className="px-6 py-4 border-b border-stroke-soft bg-bg-surface-alt flex items-center justify-between">
            <h2 className="text-sm font-bold text-fg-primary flex items-center gap-2">
              <Icon name="bell" className="w-4 h-4 text-tops-red" /> Alertas Financieras Materiales
            </h2>
            <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-rose-50 text-tops-red dark:bg-rose-950/60 dark:text-rose-400">
