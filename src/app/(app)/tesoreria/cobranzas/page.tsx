...
import { listCustomerOpenItems, getCustomerCurrentAccount, listBankAccounts } from "@/lib/tesoreria/data";
    ...
    return (
      <div className="p-4 lg:p-8 nx-page-fade">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <Kpi label="Cobranzas pendientes" value={pendiente} />
        </div>
              ...
              <tbody>
                ...
                {openItems.map((it) => (
                  <tr key={it.invoice_id} className="border-t">
                    <td className="py-2 tabular">#{it.numero_comprobante ?? it.invoice_id.slice(0, 8)}</td>
                    <td className="py-2 text-right tabular">{fmtCurrency(it.total)}</td>
                    <td className="py-2 text-right tabular">{fmtCurrency(it.saldo)}</td>
                    <td className="py-2"><StatusPill status={it.estado_cobro} /></td>
                    <td className="py-2">{fmtDate(it.fch_vto_pago)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
